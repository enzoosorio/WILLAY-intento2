...
// Chat interno — comunicación entre Super Admin y Serenazgo
// ════════════════════════════════════════════════════════════════════
...
interface Message {
  id: string; text: string; senderName: string;
  senderId: string; type: "admin" | "operator"; createdAt: any;
}
  ...
  return (
    ...
    >
      ...
      <View style={styles.header}>
        ...
        <View>
          <Text style={styles.headerTitle}>Chat — Torre de Control</Text>
          <View style={styles.onlineRow}>
            <View style={styles.onlineDot} />
            <Text style={styles.onlineTxt}>Super Admin en línea</Text>
          </View>
        </View>
      </View>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyWrap}>
            ...
            <Text style={styles.emptyTxt}>Sin mensajes aún.</Text>
            <Text style={styles.emptySub}>Escribe al Super Admin aquí.</Text>
          </View>
        }
        renderItem={({ item }) => {
          const isMe = item.senderId === user?.uid || item.type === "operator";
          return (
            <View style={[styles.msgRow, isMe && styles.msgRowMe]}>
              ...
              <View style={{ maxWidth: "75%" }}>
                {!isMe && (
                  <Text style={styles.senderName}>{item.senderName} · {formatTime(item.createdAt)}</Text>
                )}
                <View style={[styles.bubble, isMe ? styles.bubbleMe : styles.bubbleOther]}>
                  <Text style={[styles.bubbleTxt, isMe && { color: "white" }]}>{item.text}</Text>
                </View>
                {isMe && (
                  <Text style={[styles.senderName, { textAlign: "right" }]}>{formatTime(item.createdAt)}</Text>
                )}
              </View>
              {isMe && (
...
const styles = StyleSheet.create({
  ...
  meAvatar: {
    width: 28, height: 28, borderRadius: 14,
    backgroundColor: colors.warning, alignItems: "center", justifyContent: "center", flexShrink: 0,
  },
