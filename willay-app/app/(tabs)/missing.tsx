// Feed de fichas de personas desaparecidas + accesos a "crear ficha" y "escanear".
import { useEffect, useState } from "react";
import { FlatList, Image, Pressable, StyleSheet, Text, View } from "react-native";
import { onSnapshot, type QueryDocumentSnapshot } from "firebase/firestore";
import { Link } from "expo-router";
import { Ionicons } from "@expo/vector-icons";

import { Screen } from "@/components/ui/Screen";
import { activeMissingQuery } from "@/lib/collections";
import { colors } from "@/theme/colors";
import type { MissingPersonDoc } from "@/types/models";

type Row = { id: string; data: MissingPersonDoc };

export default function MissingFeed() {
  const [rows, setRows] = useState<Row[]>([]);

  useEffect(() => {
    return onSnapshot(activeMissingQuery(), (snap) =>
      setRows(snap.docs.map((d: QueryDocumentSnapshot<MissingPersonDoc>) => ({ id: d.id, data: d.data() }))),
    );
  }, []);

  return (
    <Screen padded={false}>
      <View style={styles.actions}>
        <Link href="/missing/new" asChild>
          <Pressable style={[styles.action, { backgroundColor: colors.brand }]}>
            <Ionicons name="add" size={18} color={colors.text} />
            <Text style={styles.actionText}>Nueva ficha</Text>
          </Pressable>
        </Link>
        <Link href="/missing/scan" asChild>
          <Pressable style={[styles.action, { backgroundColor: colors.surfaceAlt }]}>
            <Ionicons name="camera" size={18} color={colors.text} />
            <Text style={styles.actionText}>Escanear</Text>
          </Pressable>
        </Link>
      </View>

      <FlatList
        data={rows}
        keyExtractor={(r) => r.id}
        contentContainerStyle={styles.list}
        ListHeaderComponent={<Text style={styles.h1}>Personas buscadas</Text>}
        ListEmptyComponent={<Text style={styles.empty}>No hay fichas activas.</Text>}
        renderItem={({ item }) => (
          <Link href={{ pathname: "/missing/[id]", params: { id: item.id } }} asChild>
            <Pressable style={styles.card}>
              {item.data.photoUrl ? (
                <Image source={{ uri: item.data.photoUrl }} style={styles.avatar} />
              ) : (
                <View style={[styles.avatar, styles.avatarPlaceholder]}>
                  <Ionicons name="person" size={28} color={colors.textMuted} />
                </View>
              )}
              <View style={{ flex: 1 }}>
                <Text style={styles.name}>{item.data.name}, {item.data.age} años</Text>
                <Text style={styles.desc} numberOfLines={2}>{item.data.description}</Text>
              </View>
            </Pressable>
          </Link>
        )}
      />
    </Screen>
  );
}

const styles = StyleSheet.create({
  actions: { flexDirection: "row", gap: 10, padding: 20, paddingBottom: 0 },
  action: { flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6, paddingVertical: 12, borderRadius: 10 },
  actionText: { color: colors.text, fontWeight: "700" },
  list: { padding: 20, gap: 10 },
  h1: { color: colors.text, fontSize: 22, fontWeight: "800", marginBottom: 6 },
  empty: { color: colors.textMuted, textAlign: "center", marginTop: 40 },
  card: { flexDirection: "row", gap: 12, backgroundColor: colors.surface, borderRadius: 10, borderWidth: 1, borderColor: colors.border, padding: 12 },
  avatar: { width: 64, height: 64, borderRadius: 32, backgroundColor: colors.surfaceAlt },
  avatarPlaceholder: { alignItems: "center", justifyContent: "center" },
  name: { color: colors.text, fontWeight: "700" },
  desc: { color: colors.textMuted, fontSize: 12, marginTop: 2 },
});
